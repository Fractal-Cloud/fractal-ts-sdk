...
import {ApiManagementSaaSUnmanaged} from './unmanaged';
...
describe('ApiManagementSaaSUnmanaged', () => {
  describe('create()', () => {
    it('should build a component with the correct type string', () => {
      const c = ApiManagementSaaSUnmanaged.create(BASE_CONFIG);
  ...
  describe('satisfy()', () => {
    it('should copy id, version, displayName from blueprint', () => {
      const bp = ApiManagementUnmanaged.create({
        id: 'bp-unmanaged',
        version: {major: 2, minor: 1, patch: 3},
        displayName: 'Blueprint Unmanaged',
      });

      const c = ApiManagementSaaSUnmanaged.satisfy(bp.component).build();
