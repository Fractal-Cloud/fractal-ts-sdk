...
const ARUBA_MSSQL_DBMS_TYPE = buildType();

/**
 * Aruba MS SQL DBMS — Aruba-managed relational DBMS Offer satisfying the abstract
 * RelationalDbms. Inherits the vendor-neutral `version` and `flavorName`
 * parameters. It has no vendor-only extras.
 */
export const ArubaMsSqlDbms: Offer = {
  type: ARUBA_MSSQL_DBMS_TYPE,
  provider: 'Aruba',
  instantiate: ctx => [
    instantiateFromNeutral(ctx, ARUBA_MSSQL_DBMS_TYPE, 'Aruba'),
  ],
};
