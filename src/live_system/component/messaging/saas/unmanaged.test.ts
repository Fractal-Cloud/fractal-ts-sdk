...
import {MessagingSaaSUnmanaged} from './unmanaged';
...
describe('MessagingSaaSUnmanaged', () => {
  describe('create()', () => {
    it('should build a component with the correct type string', () => {
      const c = MessagingSaaSUnmanaged.create(BASE_CONFIG);
  ...
  describe('satisfy()', () => {
    it('should copy id, version, displayName from blueprint', () => {
      const bp = MessagingUnmanaged.create({
        id: 'bp-unmanaged',
        version: {major: 2, minor: 1, patch: 3},
        displayName: 'Blueprint Unmanaged',
      });

      const c = MessagingSaaSUnmanaged.satisfy(bp.component).build();
