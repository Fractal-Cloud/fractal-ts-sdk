...
import {SecuritySaaSUnmanaged} from './unmanaged';
...
describe('SecuritySaaSUnmanaged', () => {
  describe('create()', () => {
    it('should build a component with the correct type string', () => {
      const c = SecuritySaaSUnmanaged.create(BASE_CONFIG);
  ...
  describe('satisfy()', () => {
    it('should copy id, version, displayName from blueprint', () => {
      const bp = SecurityUnmanaged.create({
        id: 'bp-unmanaged',
        version: {major: 2, minor: 1, patch: 3},
        displayName: 'Blueprint Unmanaged',
      });

      const c = SecuritySaaSUnmanaged.satisfy(bp.component).build();
