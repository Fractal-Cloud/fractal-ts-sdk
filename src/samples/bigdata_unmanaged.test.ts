/**
 * bigdata_unmanaged.test.ts
 *
 * M4 migration proof for the BigData "Unmanaged" capability under the Fractal +
 * Interface model. Mirrors samples/storage_relationaldatabase.test.ts and
 * samples/messaging_broker.test.ts:
 *
 *   - An infra team authors ONE Fractal whose BigDataUnmanaged abstract component
 *     carries the candidate Offer (BigDataSaaSUnmanaged/SaaS).
 *   - A dev specializes through the Interface only, never naming a vendor offer.
 *     Unmanaged carries no vendor-neutral knobs, so specialization is limited to
 *     declared dependencies and links flowing through to the selected offer.
...
import {BigDataSaaSUnmanaged} from '../live_system/component/big_data/saas/unmanaged';
...
function authorUnmanagedFractal() {
  return createFractal({
    ...
    description: 'Governed unmanaged big-data system',
    boundedContextId,
    blueprint: bp => ({
      unmanaged: bp.add(
        BigDataUnmanaged.create({
          id: 'unmanaged',
          displayName: 'External Big Data System',
          offers: [BigDataSaaSUnmanaged],
...
describe('BigDataUnmanaged Fractal — provider-driven offer swap', () => {
  ...
  it('serializes candidate offers onto the Blueprint component Service', () => {
    ...
    expect(serviceTypes).toEqual(['BigData.SaaS.Unmanaged']);

    const offerTypes = unmanaged
      .services!.flatMap(s => s.offers)
      .map(o => o.type.toString())
      .sort();
    expect(offerTypes).toEqual(['BigData.SaaS.Unmanaged']);
  });
